...
%       clean = enh.enhance(mic);     % mic = [L x N], clean = [L x 1]
    ...
    methods
        ...
        function [clean, info] = enhance_metered(obj, mic, board_w, threshold_mw)
        %ENHANCE_METERED  Like enhance(), but also returns a power estimate
        %   (run_demo.py energy-meter algorithm) in `info`:
        %     info.power_mw, .threshold_mw, .rtf, .latency_ms, .audio_s,
        %     .src, .os
            if nargin < 3 || isempty(board_w),      board_w = 7.0;  end
            if nargin < 4 || isempty(threshold_mw), threshold_mw = 50.0; end
            info = struct();
            if isempty(mic)
                clean = zeros(0, 1);
                return;
            ...
            obj.ensure_helper_();
            [L, N] = size(mic);
            fprintf(['[Q-WiSE] ONNX enhance (metered): %d mic channel(s) ' ...
                     'x %d samples\n'], N, L);

            pa  = py.array.array('d', mic(:));
            res = obj.helper_.enhance_metered(obj.model_path, pa, ...
                      int64(L), int64(N), board_w, threshold_mw);

            clean = double(res.get('clean'));
            clean = clean(:);
            if numel(clean) >= L
                clean = clean(1:L);
            end
            info.power_mw     = double(res.get('power_mw'));
            info.threshold_mw = double(res.get('threshold_mw'));
            info.rtf          = double(res.get('rtf'));
            info.latency_ms   = double(res.get('latency_ms'));
            info.audio_s      = double(res.get('audio_s'));
            info.src          = char(res.get('src'));
            info.os           = char(res.get('os'));
        end
    end
